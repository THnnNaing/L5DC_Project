...
if(isset($_POST['btnSearch'])) {
    ...
    if($rdoSearchType == 1) {
        $lstProductID = mysqli_real_escape_string($db, $_POST['lstProductID']);
        $query = "SELECT o.*, c.customer_name
                  FROM order_tb o, customer_tb c
                  WHERE o.order_id='$lstProductID'
        ...
        $query = "SELECT o.*, c.customer_name
                  FROM order_tb o, customer_tb c
                  WHERE o.order_date BETWEEN '$txtFrom' AND '$txtTo'
                  AND o.customer_id=c.customer_id";
        $result = mysqli_query($db, $query);
        $count = mysqli_num_rows($result);
    } else {
        $cboStatus = mysqli_real_escape_string($db, $_POST['cboStatus']);
        $query = "SELECT o.*, c.customer_name
                  FROM order_tb o, customer_tb c
                  WHERE o.order_status='$cboStatus'
                  AND o.customer_id=c.customer_id";
        $result = mysqli_query($db, $query);
        $count = mysqli_num_rows($result);
    }
...
<div class="w-32 h-screen fixed">
    ...
    <form action="orderlist.php" method="post" class="space-y-5">
    <fieldset class="border border-yellow-500 p-4 rounded">
        ...
        <div class="space-y-4">
            <div class="flex items-center space-x-4">
                ...
                <label for="searchById" class="text-yellow-800">Search By Order ID</label>
    ...
    <fieldset class="border border-yellow-500 p-4 rounded">
        ...
        if($count<1) 
        {
            echo "<p class='text-red-500'>No Order Found.</p>";
        }
        else
        {
        ?>
            <table class="min-w-full bg-white border border-yellow-500 rounded">
                <thead class="bg-yellow-500 text-white">
                    <tr>
                        ...
                        <th class="py-2 px-4 border">Order Total Amount</th>
                        <th class="py-2 px-4 border">Order Tax</th>
                        <th class="py-2 px-4 border">Order All Total</th>
                        <th class="py-2 px-4 border">Order Total Quantity</th>
                        <th class="py-2 px-4 border">Remark</th>
                        <th class="py-2 px-4 border">Payment Type</th>
                        <th class="py-2 px-4 border">Order Location</th>
                        <th class="py-2 px-4 border">Order Phonenumber</th>
                ...
                <tbody>
                    ...
                    { 
                        ...
                        echo "<td class='py-2 px-4 border'>$Order_id</td>";
                        echo "<td class='py-2 px-4 border'>" . $row['order_date'] . "</td>";
                        echo "<td class='py-2 px-4 border'>" . $row['orderTotalAmount'] . "</td>";
                        echo "<td class='py-2 px-4 border'>" . $row['orderTax'] . "</td>";
                        echo "<td class='py-2 px-4 border'>" . $row['orderAllTotal'] . "</td>";
                        echo "<td class='py-2 px-4 border'>" . $row['orderTotalQuantity'] . "</td>";
                        echo "<td class='py-2 px-4 border'>" . $row['remark'] . "</td>";
                        echo "<td class='py-2 px-4 border'>" . $row['payment_type'] . "</td>";
                        echo "<td class='py-2 px-4 border'>" . $row['order_location'] . "</td>";
                        echo "<td class='py-2 px-4 border'>" . $row['order_phone'] . "</td>";
                        echo "<td class='py-2 px-4 border'>" . $row['order_status'] . "</td>";
                        echo "<td class='py-2 px-4 border'>" . $row['customer_name'] . "</td>";
                        echo "<td class='py-2 px-4 border'><a href='orderdetails.php?order_id=$Order_id' class='text-yellow-600 hover:underline'>Details</a></td>";
                        echo "</tr>";
                    }			
                    ?>
                </tbody>
