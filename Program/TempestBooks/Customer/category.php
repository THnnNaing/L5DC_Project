...
$categoryQuery = "SELECT * FROM category_tb";
...
if ($category) {
    $sql .= " WHERE b.category_id = '" . $db->real_escape_string($category) . "'";
}
        ...
        <div class="container mx-auto text-center">
            ...
            <div class="flex justify-center">
                <form action="" method="GET" class="flex w-full max-w-2xl">
                    <select name="category" class="w-2/3 py-3 px-4 rounded-l-full focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-black">
                        <option value="" disabled selected>Select a category...</option>
                        <?php
                        if ($categoryResult->num_rows > 0) {
                            while ($catRow = $categoryResult->fetch_assoc()) {
                                echo '<option value="' . $catRow['category_id'] . '">' . $catRow['category'] . '</option>';
                            }
                        }
                        ?>
                    </select>
